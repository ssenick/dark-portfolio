...
export function preloader() {
  ...
  function imageLoaded() {
    imagesLoadedCount++;
    //  percDisplay.innerHTML = Math.round((100 / imagesTotalCount) * imagesLoadedCount) + '%';
    percDisplay.style.setProperty('--progress', Math.round((100 / imagesTotalCount) * imagesLoadedCount) + '%');
      ...
      window.addEventListener('load', function () {
        ...
        links.forEach(function(link) {
          link.addEventListener('click', function(event) {
            ...
            // Обновляем и показываем percDisplay плавно
            percDisplay.classList.remove('is-hidden');
            percDisplay.style.transition = 'opacity 1s';
            percDisplay.style.transition = 'opacity .8s cubic-bezier(.645, .045, .355, 1)'
            percDisplay.style.setProperty('--progress', '0%')
            percDisplay.style.opacity = '1';
            const url = this.href; // Сохраняем URL
